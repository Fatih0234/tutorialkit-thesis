...
test('runs and recovers an introductory multi-file Python lesson', async ({ page }) => {
  ...
  await page.request.post('/api/interactive/auth/dev-login', {
    data: { userId: 'dev-user-teacher-demo-8f4c2a9d' },
  });
  ...
  await expect(editMaterials).toBeVisible();
  ...
  const executionControls = page.getByLabel('Python execution controls');
  await expect(executionControls).toBeVisible();
  await expect(executionControls.getByRole('status')).toContainText('ready', { timeout: 30_000 });
  ...
  // Insert in one transaction and run without waiting for the settled-change debounce.
  await page.keyboard.insertText('from helpers import greet\nprint(greet("Grace"))\n');
  await page.getByRole('button', { name: 'Run', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Run', exact: true }).click();
  await expect(executionControls.getByRole('status')).toContainText('running');
