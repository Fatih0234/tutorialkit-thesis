...
- [ ] Retain the terminal output showing build success and **40 passed**.
...
These are currently non-blocking only when the build command exits successfully, the preview server becomes ready, and the full suite reports **40 passed**. Treat any different error, server startup failure, test timeout, failed assertion, or non-zero final command as a release blocker until investigated.
