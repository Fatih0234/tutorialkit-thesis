...
Recordings may optionally snapshot `presentationResources` and `initialPresentationLayout`; the layout includes optional per-deck progress and an additive semantic composition. Composition stores a preset, Main Stage and optional Sidecar surface IDs, a bounded split ratio, and constrained camera anchor/size—never raw pixel geometry. Subsequent teacher arrangement, resize-completion, camera, slide, and progressive-reveal cues are ordinary ordered `presentation.changed` snapshots. Package format 1 and remote validation accept normalized compositions and reject malformed presets, ratios, camera values, or unknown surfaces. Existing recordings without composition remain valid and map deterministically in memory; review does not rewrite them.
