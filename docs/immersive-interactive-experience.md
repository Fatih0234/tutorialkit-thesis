...
A responsive lesson compositor places the real editor and presentation resources into Main Stage, Sidecar, and Tray roles instead of overlapping windows. Focus, Side-by-side, and Stage with Sidecar presets use a bounded semantic split ratio; compact viewports stack the stages without changing recorded intent. **Arrange Layout** supports constrained drag/drop plus accessible Main/Side actions. The instructor camera is the only overlay and snaps to four anchors with named sizes. Teacher action boundaries become deterministic `presentation.changed` cues; raw pointer movement and pixel geometry are excluded. Learner controls create temporary local overrides; **Follow Teacher** or the next teacher cue restores teacher direction. The same TutorialKit editor, preview iframe, terminal, whiteboard, and media owners remain mounted across composition changes. See `docs/presentation-resource-layer.md`.
