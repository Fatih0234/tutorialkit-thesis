...
A presentation layer in the workspace content region can independently minimize, focus, hide, and reopen the existing live website preview, lesson explanation, whiteboard, and snapshot-safe multi-slide deck. Its Resources launcher occupies a dedicated, horizontally scrollable row below the Workspace controls, so it never covers the editor header or first code line. Presentation windows are bounded below both permanent toolbar rows. The Resources toolbar opens a fixed right preview window and layered same-position left windows; selecting a covered left resource brings it forward without stacking or discarding its state. Teacher actions during recording become deterministic `presentation.changed` cues. Learner controls create temporary local overrides; **Follow teacher** or the next teacher cue restores teacher direction. The same TutorialKit preview iframe remains mounted and interactive across presentation modes. See `docs/presentation-resource-layer.md`.
