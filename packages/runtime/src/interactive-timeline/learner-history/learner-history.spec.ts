...
describe('learner history', () => {
  ...
  it('materializes file changes, creation, deletion, and rename', () => {
    const events = [
      ...
      },
      {
        ...
        seq: 3,
        createdAt: '3',
        type: 'file.renamed' as const,
        payload: { from: 'b.js', to: 'c.js' },
      },
      {
        schemaVersion: 1 as const,
        id: '4',
  ...
  it('forks historical state without changing the parent branch', () => {
    const parent = createLearnerBranch({
      id: 'parent',
      userId: 'u',
      lessonId: 'l',
      origin,
    ...
    const second = appendLearnerHistoryEvent({
      branch: first.branch,
      id: 'p2',
      type: 'file.changed',
      filePath: '/a',
    ...
    const childEvent = appendLearnerHistoryEvent({
      branch: child.branch,
      id: 'c1',
      type: 'file.changed',
      filePath: '/a',
      payload: { content: 'forked' },
    });
    ...
    expect(
      materializeLearnerBranchGraph({ '/a': 'base' }, childEvent.branch, [second.branch, childEvent.branch], events),
    ).toEqual({ '/a': 'forked' });
    expect(
      materializeLearnerBranchGraph({ '/a': 'base' }, second.branch, [second.branch, childEvent.branch], events),
    ).toEqual({ '/a': 'two' });
  ...
  it('creates commits only when the working tree differs from the latest commit', () => {
    ...
    const first = createLearnerCommit({
      branch: created.branch,
      workingTree: created.workingTree,
      name: 'calm-orbit',
      id: 'c1',
    });
  ...
  it('rejects malformed event ordering', () => {
    ...
    ).toThrow(/ascending positive sequences/);
  });
});
