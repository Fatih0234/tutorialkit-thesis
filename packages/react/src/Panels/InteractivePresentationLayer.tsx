...
export function InteractivePresentationLayer({ resources, layout, explanationHtml, canEditDeck, onModeChange, onDeckAction, onDeckChange, onPreviewHostChange, cameraMediaUrl, onCameraMediaElementRef, whiteboardScene, whiteboardReadOnly, whiteboardError, onWhiteboardSceneCommit }: Props) {
  ...
  return (
    <div data-presentation-layer className="pointer-events-none absolute inset-0 z-20 overflow-hidden">
      ...
      <div data-presentation-window-side="left" className={`absolute left-4 h-64 w-[min(26rem,42vw)] ${frontmostLeftId === whiteboardResource?.id ? 'z-10' : 'z-20'} ${cameraMode === 'minimized' && cameraMediaUrl ? 'bottom-60' : 'bottom-4'}`}>
        {resources.filter((resource) => resource.kind !== 'preview' && resource.kind !== 'camera' && resource.kind !== 'whiteboard' && layout.resources[resource.id] === 'minimized').map((resource) => (
          <PresentationFrame key={resource.id} resource={resource} mode="minimized" frontmost={frontmostLeftId === resource.id} onModeChange={onModeChange}>
      ...
      {previewResource ? <section aria-label="Website preview presentation" data-presentation-resource={previewResource.id} data-presentation-mode={previewMode} data-presentation-frontmost={layout.frontmostBySide?.right === previewResource.id} className={previewContainerClass(previewMode)}><ResourceHeader resource={previewResource} mode={previewMode} onModeChange={onModeChange} /><div ref={onPreviewHostChange} data-presentation-preview-host className="min-h-0 flex-1 overflow-hidden bg-white" /></section> : null}
      {whiteboardResource ? <section aria-label="Whiteboard presentation" aria-hidden={whiteboardMode === 'hidden'} data-presentation-resource={whiteboardResource.id} data-presentation-mode={whiteboardMode} data-presentation-frontmost={frontmostLeftId === whiteboardResource.id} className={whiteboardContainerClass(whiteboardMode, frontmostLeftId === whiteboardResource.id, cameraMode === 'minimized' && Boolean(cameraMediaUrl))}><ResourceHeader resource={whiteboardResource} mode={whiteboardMode} onModeChange={onModeChange} /><div className="min-h-0 flex-1"><InteractiveWhiteboard scene={whiteboardScene} readOnly={whiteboardReadOnly} error={whiteboardError} onSceneCommit={onWhiteboardSceneCommit} /></div></section> : null}
...
function DeckContent({ deck, progress, compact, canEdit, onAction, onChange }: { deck: DeckPresentationResource; progress?: DeckPlaybackState; compact: boolean; canEdit: boolean; onAction: (deckId: string, action: DeckAction, slideIndex?: number) => void; onChange: (deck: DeckPresentationResource) => void }) {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-gradient-to-br from-indigo-100 to-violet-200 text-slate-950">
    ...
    <div className={`shrink-0 border-t border-indigo-200 bg-white/70 ${compact ? 'px-3 py-2' : 'px-5 py-3'}`}>
      ...
      {!compact ? <div className="mt-2 flex flex-wrap items-center justify-center gap-2"><InteractiveButton variant="ghost" icon="i-ph-caret-double-left" data-pointer-anchor={`deck:${deck.id}:previous-slide`} onClick={() => onAction(deck.id, 'previous-slide')}>Previous slide</InteractiveButton><InteractiveButton variant="ghost" icon="i-ph-caret-left" data-pointer-anchor={`deck:${deck.id}:previous-reveal`} onClick={() => onAction(deck.id, 'previous-reveal')}>Previous step</InteractiveButton><InteractiveButton variant="primary" icon="i-ph-caret-right" data-pointer-anchor={`deck:${deck.id}:next-reveal`} onClick={() => onAction(deck.id, 'next-reveal')}>Reveal next</InteractiveButton><InteractiveButton variant="ghost" icon="i-ph-caret-double-right" data-pointer-anchor={`deck:${deck.id}:next-slide`} onClick={() => onAction(deck.id, 'next-slide')}>Next slide</InteractiveButton></div> : null}
