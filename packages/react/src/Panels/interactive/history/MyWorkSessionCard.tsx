...
export function MyWorkSessionCard({
  group,
  current,
  onOpen,
}: {
  ...
  return (
    <article
      className={`rounded-lg border bg-tk-background-secondary p-3 transition ${current ? 'border-orange-400 ring-1 ring-orange-400/30' : 'border-tk-border-primary'}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <strong className="block text-sm text-tk-text-primary">
            {hasDraft ? 'Autosaved draft' : 'Checkpointed work'}
          </strong>
          <span className="text-xs text-tk-text-secondary">
            Started at lesson {formatInteractiveTime(group.teacherTimestampMs)}
          </span>
        </div>
        {current ? (
          <span className="rounded-full bg-orange-500/15 px-2 py-1 text-[0.65rem] font-semibold text-orange-200">
            Currently open
          </span>
        ) : null}
      </div>
          ...
          <ul className="m-0 mt-2 grid list-none gap-1 p-0">
            {changes.map((change) => (
              <li key={change.path} className="flex items-center justify-between gap-2 text-xs">
                <span className="min-w-0 truncate font-mono text-tk-text-primary" title={change.path}>
      ...
      <div className="mb-3 flex flex-wrap gap-x-2 gap-y-1 text-[0.68rem] text-tk-text-secondary">
        <span>
          {group.checkpointCount} checkpoint{group.checkpointCount === 1 ? '' : 's'}
        </span>
        <span aria-hidden="true">·</span>
        <span>
          {alternatives === 0 ? '1 version path' : `${alternatives} alternative path${alternatives === 1 ? '' : 's'}`}
        </span>
        {group.dirtyHeadCount > 0 ? (
          <>
            <span aria-hidden="true">·</span>
            <span>
              {group.dirtyHeadCount} draft{group.dirtyHeadCount === 1 ? '' : 's'}
            </span>
          </>
        ) : null}
      </div>
      ...
      <button
        type="button"
        onClick={onOpen}
        className="flex w-full items-center justify-center gap-1.5 rounded bg-orange-500 px-3 py-2 text-xs font-semibold text-white transition hover:bg-orange-400 active:scale-[0.98] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
      >
...
export function getWorkGroupUpdatedAt(group: LearnerTimelineGroup) {
  return (
    group.branches
      .flatMap((branch) => [branch.headUpdatedAt, ...branch.commits.map((commit) => commit.createdAt)])
