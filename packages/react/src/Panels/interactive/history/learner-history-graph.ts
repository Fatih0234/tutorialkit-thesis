...
export function getLatestLearnerGraphSelection(group: LearnerTimelineGroup): LatestLearnerGraphSelection | undefined {
  const candidates: Array<LatestLearnerGraphSelection & { timestamp: string }> = group.branches.flatMap((branch): Array<LatestLearnerGraphSelection & { timestamp: string }> => {
    if (branch.dirty) {
      return [{ branchId: branch.branchId, kind: 'head' as const, timestamp: branch.headUpdatedAt }];
    }
    const latestCommit = [...branch.commits].sort((a, b) =>
      b.createdAt.localeCompare(a.createdAt) || b.id.localeCompare(a.id))[0];
    return latestCommit
      ? [{ branchId: branch.branchId, kind: 'commit' as const, commitId: latestCommit.id, timestamp: latestCommit.createdAt }]
      : [];
  });

  const latest = candidates.sort((a, b) =>
    b.timestamp.localeCompare(a.timestamp) || b.branchId.localeCompare(a.branchId))[0];
...
export function buildLearnerTimelineGroups(branches: LearnerTimelineBranchSummary[]): LearnerTimelineGroup[] {
  ...
  for (const branch of branches) {
    if (branch.commits.length === 0 && !branch.dirty) continue;
...
export function buildLearnerHistoryGraph(group: LearnerTimelineGroup): LearnerHistoryGraph {
  ...
  const visit = (branch: LearnerTimelineBranchSummary) => {
    ordered.push(branch);
    for (const child of children.get(branch.branchId) ?? []) visit(child);
  };
  ...
  ordered.forEach((branch, row) => {
    const y = 24 + row * 64;
    const parentAnchor = branch.parent ? findParentAnchor(nodesByBranch.get(branch.parent.branchId), branch.parent) : undefined;
    ...
    const commits = [...branch.commits].sort((a, b) => a.eventSeq - b.eventSeq || a.createdAt.localeCompare(b.createdAt));
    ...
    branchNodes.push({
      ...
      label: branch.parent
        ...
        : branch.dirty ? 'Autosaved draft' : 'Latest work',
      dirty: branch.dirty,
    });
...
function findParentAnchor(
  ...
  if (!parentNodes?.length) return undefined;
  if (parent.commitId) {
    const exactCommit = parentNodes.find((node) => node.commitId === parent.commitId);
    if (exactCommit) return exactCommit;
  }
