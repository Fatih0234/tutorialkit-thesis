...
export const ExcalidrawCanvas = memo(function ExcalidrawCanvas({ scene, readOnly, error, onSceneCommit }: Props) {
  const apiRef = useRef<{
    updateScene: (scene: ReturnType<typeof toExcalidrawInitialData>) => void;
    refresh?: () => void;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => requestAnimationFrame(() => apiRef.current?.refresh?.()));
  ...
  return (
    <div
      ...
      data-whiteboard-readonly={readOnly}
      data-whiteboard-element-count={scene.elements.filter((element) => !element.isDeleted).length}
    ...
    />
      <span className="sr-only" aria-live="polite">
        Whiteboard scene contains {scene.elements.filter((element) => !element.isDeleted).length} elements.
      </span>
      {error ? (
        <div
          role="alert"
          className="absolute bottom-2 left-2 right-2 rounded bg-red-950/90 px-3 py-2 text-xs text-white"
        >
          {error}
        </div>
      ) : null}
