...
import {
  countCheckpoints,
  countDrafts,
  getWorkGroupUpdatedAt,
  MyWorkSessionCard,
} from './interactive/history/MyWorkSessionCard.js';
import {
...
export function InteractiveVideoControls({ audience, model, onPlay, onPause }: InteractiveVideoControlsProps) {
  ...
  return (
    <>
      <footer
        aria-label="Interactive lesson controls"
        className="relative z-20 shrink-0 border-t border-tk-border-primary bg-tk-background-primary px-4 pb-3 pt-2 shadow-[0_-8px_30px_rgba(0,0,0,0.18)]"
      >
        <div hidden>
        ...
        <div className="mx-auto grid max-w-screen-2xl gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <InteractiveButton
              ...
              onClick={model.onRestartPlayback}
                  ...
                  {isLearnerWorkspace ? (
                    <>
                      ...
                      <span className="text-xs text-tk-text-secondary">
                        Ctrl/Cmd+S creates a checkpoint · Play resumes the lesson
                      </span>
                    </>
                  ) : (
                    <span className="text-xs text-tk-text-secondary">
                      Edit code to pause automatically and create your branch
                    </span>
                  )}
                  <InteractiveButton
      ...
      {isLearner && historyOpen ? (
        <aside
          role="dialog"
          aria-modal="true"
          aria-labelledby="learner-work-title"
          className="fixed bottom-0 right-0 top-14 z-[1020] flex w-[min(27rem,94vw)] flex-col border-l border-tk-border-primary bg-tk-background-primary p-4 shadow-2xl"
        >
          <div className="flex items-start justify-between gap-3 border-b border-tk-border-primary pb-3">
            <div>
              <h2 id="learner-work-title" className="m-0 text-base font-700 text-tk-text-primary">
                My Work
              </h2>
              <p className="m-0 mt-1 text-xs text-tk-text-secondary">
                Your drafts are autosaved. Ctrl/Cmd+S creates a checkpoint.
              </p>
              <p className="m-0 mt-2 text-xs font-medium text-tk-text-primary">
                {workGroups.length} work session{workGroups.length === 1 ? '' : 's'} · {countCheckpoints(workGroups)}{' '}
                checkpoint{countCheckpoints(workGroups) === 1 ? '' : 's'} · {countDrafts(workGroups)} autosaved draft
                {countDrafts(workGroups) === 1 ? '' : 's'}
              </p>
            </div>
            <InteractiveButton
          ...
          <div className="min-h-0 flex-1 overflow-y-auto py-3">
            {model.learnerRemoteSyncStatus.includes('offline') || model.learnerRemoteSyncStatus.includes('failed') ? (
              <p
                role="status"
                className="mb-3 rounded border border-amber-500/40 bg-amber-950/20 p-2 text-xs text-amber-200"
              >
            ...
            {workItemCount === 0 ? (
              <div className="rounded-lg border border-dashed border-tk-border-primary p-5 text-center">
                <span
                  className="i-ph-pencil-simple-line mb-2 inline-block text-2xl text-orange-300"
                  aria-hidden="true"
                />
                <p className="m-0 text-sm font-medium text-tk-text-primary">No work yet</p>
                <p className="m-0 mt-1 text-xs text-tk-text-secondary">
                  Edit a file during the lesson to create your first autosaved work session.
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
                {sortedWorkGroups.map((group) => (
                  <MyWorkSessionCard
                    ...
                    group={group}
                    current={
                      model.workspaceOwner === 'learner' &&
                      group.branches.some((branch) => branch.branchId === model.activeLearnerBranchId)
                    }
                ...
                {model.learnerCheckpoints.map((checkpoint) => (
                  <article
                    key={checkpoint.id}
                    className="rounded-lg border border-tk-border-primary bg-tk-background-secondary p-3"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <strong className="block text-sm text-tk-text-primary">Imported checkpoint</strong>
                        <span className="text-xs text-tk-text-secondary">
                          Started at lesson {formatInteractiveTime(checkpoint.teacherTimestampMs)}
                        </span>
                      </div>
                      <span className="rounded-full bg-violet-500/15 px-2 py-1 text-[0.65rem] font-medium text-violet-200">
                        Legacy
                      </span>
                    </div>
                    <p className="my-2 text-xs text-tk-text-secondary">
                      {checkpoint.changedFileCount} changed file{checkpoint.changedFileCount === 1 ? '' : 's'} ·{' '}
                      {checkpoint.versionCount} saved version{checkpoint.versionCount === 1 ? '' : 's'}
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                        model.onOpenLearnerCheckpoint(checkpoint.id);
                        setHistoryOpen(false);
                        historyButtonRef.current?.focus();
                      }}
                      className="w-full rounded bg-violet-500 px-3 py-2 text-xs font-semibold text-white transition hover:bg-violet-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
                    >
                      Open work
                    </button>
                  </article>
